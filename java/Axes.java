...
public class Axes extends ControlThread {
     ...
     * @param ra Source RA (rads).
     * @param dec Source dec (rads).
     */
	...
	try { Thread.sleep(interval); } catch (InterruptedException ix) {System.err.println("Axes::interrupted");}
	...
	System.err.println("Axes::Azimuth demand: "+telescope.mechanisms.azDemand+", Current: "+telescope.mechanisms.azPos);
    }
	...
	telescope.mechanisms.altDemand  = Math.toDegrees(alt);
	System.err.println("Axes::Altitude demand: "+telescope.mechanisms.altDemand+", Current: "+telescope.mechanisms.altPos);
    }
	...
	para = -para; // rads.
	System.err.println("Paralytic angle is: "+Position.toDegrees(para, 3));
	...
	case TCS_Status.ROT_SKY:
	    rot = correct(2*Math.PI+para+Math.toRadians(rotator)); // (0, 2*PI).
	    telescope.mechanisms.rotSkyAngle = rotator;
	    System.err.println("Setting skyangle to: "+telescope.mechanisms.rotSkyAngle);	
	...
	case TCS_Status.ROT_VFLOAT: 
		...
		telescope.mechanisms.rotSkyAngle = Math.toDegrees(2.0*Math.PI-para);
	    break;
	}
	telescope.mechanisms.rotDemand  = wrot;
	System.err.println("Axes::Rotator demand: "+wrot+", Current: "+telescope.mechanisms.rotPos);
    }
	...
	if (daz > 0.0 && daz < 180.0) {
		...
		System.err.println("Axes::Azwrap::Single wrap az-slew LEFT by"+(telescope.mechanisms.azPos - waz));
	    } else {
		ttaz = (waz - telescope.mechanisms.azPos)/2.0;
	
		System.err.println("Axes::Azwrap::Single wrap az-slew RIGHT by"+(waz - telescope.mechanisms.azPos));
	    }
	}
	else {
	    // 2 Choices.
		...
		waz = waz2;
		ttaz  = deltaR/2.0;
		System.err.println("Axes::Azwrap::Must az-slew RIGHT "+deltaR+" ("+deltaL+") to limit at "+limL);
	    } 
	    else if
		(deltaR > limR) {
		...
		waz = waz1;
		ttaz = deltaL/2.0;
		System.err.println("Axes::Azwrap::Must az-slew LEFT "+deltaL+" ("+deltaR+") to limit at "+limR);
	    } else {
		if (deltaL < deltaR) {
		  
		    waz = waz1;
		    ttaz = deltaL/2.0;
		    System.err.println("Axes::Azwrap::Shortest az-Slew LEFT "+deltaL+" ("+deltaR+")");
		} else {
		  
		    waz = waz2;
		    ttaz  = deltaR/2.0;
		    System.err.println("Axes::Azwrap::Shortest az-Slew RIGHT "+deltaR+" ("+deltaL+")");
		}
	    }
	}
	...
	if ((drot > 240.0 && drot < 360.0) ||
		...
		System.err.println("Axes::Rotwrap::Single wrap rot-slew LEFT by"+(telescope.mechanisms.rotPos - wrot));
	    } else {
		ttrot = (wrot - telescope.mechanisms.rotPos)/2.0;
		
		System.err.println("Axes::Rotwrap::Single wrap rot-slew RIGHT by"+(wrot - telescope.mechanisms.rotPos));
	    }
	}
	else {
	    // 2 Choices.
		...
		wrot = wrot2;
		ttrot  = deltaR/2.0;
		System.err.println("Axes::Rotwrap::Must rot-slew RIGHT "+deltaR+" ("+deltaL+") to limit at "+limL);
	    } 
	    else if
		(deltaR > limR) {
		...
		wrot = wrot1;
		ttrot = deltaL/2.0;
		System.err.println("Axes::Rotwrap::Must rot-slew LEFT "+deltaL+" ("+deltaR+") to limit at "+limR);
	    } else {
		if (deltaL < deltaR) {
		   
		    wrot = wrot1;
		    ttrot = deltaL/2.0;
		    System.err.println("Axes::Rotwrap::Shortest rot-Slew LEFT "+deltaL+" ("+deltaR+")");
		} else {
		  
		    wrot = wrot2;
		    ttrot  = deltaR/2.0;
		    System.err.println("Axes::Rotwrap::Shortest rot-Slew RIGHT "+deltaR+" ("+deltaL+")");
		}
	    }
	}
	...
	double delta = Math.abs(telescope.mechanisms.azPos - telescope.mechanisms.azDemand);

	System.err.println("Axes::StepAz::AzDmd: "+telescope.mechanisms.azDemand+
			   ", AzPos:" +telescope.mechanisms.azPos+
			   ", Delta: "+delta+
			   ", MinDelta: "+ee);
	...
	double ee = 3.0*interval/1000.0;
	double delta = Math.abs(telescope.mechanisms.altPos - telescope.mechanisms.altDemand);

	System.err.println("Axes::StepAlt::AltDmd: "+telescope.mechanisms.altDemand+
			   ", AltPos:" +telescope.mechanisms.altPos+
			   ", Delta: "+delta+
			   ", MinDelta: "+ee);
	...
	double ee = 3.0*interval/1000.0;
	double delta = Math.abs(telescope.mechanisms.rotPos - telescope.mechanisms.rotDemand);

	System.err.println("Axes::StepRot::RotDmd: "+telescope.mechanisms.rotDemand+
			   ", RotPos:" +telescope.mechanisms.rotPos+
			   ", Delta: "+delta+
			   ", MinDelta: "+ee);
