...
public class Mcp extends ControlThread {
     ...
     * @param sdb Telescope status block.
   ...
   /** Request OPER ON to be performed.*/
	...
	try { Thread.sleep(interval); } catch (InterruptedException ix) {System.err.println("MCP::interrupted");}
	...
	System.err.println("MCP::Sun elev: "+Position.toDegrees(sun.getAltitude(),3));
	if (sun.isRisen())
	    opHours = false;
	...
	System.err.println("MCP::Current state:"+toStateString(state)+
			   " WMS: "+(wmsOkay ? "OKAY" : "SUSPEND")+
			   " AXES: "+(axesOkay ? "OKAY" : "SUSPEND")+
			   " HOURS: "+(opHours ? "OKAY" : "SUSPEND"));
