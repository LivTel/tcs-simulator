...
public class InstrumentControl {
    ...
    static final String[] ratlfilt = new String[] {"clear", "SDSS-I", "SDSS-Z", "SDSS-R", "H-Alpha-100"};
    static final String[] ratufilt = new String[] {"clear", "SDSS-G", "SDSS-U", "Bessell-B", "Bessell-V"};
    ...
    public void setHasArms(boolean arms) { this.hasArms = arms;}

    public void setIssHost(String h) {this.issHost = h;}

    public void setIssPort(int p) {this.issPort = p;}

    public void setAvSeeing(double a) { this.avSee = a;}

    public void setDeltaSeeing(double d) { this.deltaSee = d;}
    
    public void setMirrorPort(int p) { this.mirrorPort = p;}

    public void setSendDpAcks(boolean s) { this.sendDpAcks = sendDpAcks;}
	...
	if ( (implementor == null) ||
	     ! (implementor instanceof JMSMA_ProtocolServerImpl) ) return null;
	if ( (request == null)    || 
	...
	InstrumentRequestHandler impl = new InstrumentRequestHandler((JMSMA_ProtocolServerImpl)implementor, 
								     (COMMAND)request);
	return impl;
	}
	...
	InstrumentRequestHandler(JMSMA_ProtocolServerImpl implementor, COMMAND request) {
	...
	public void handleRequest() {
		...
		System.err.println("Config Class: "+((CONFIG)request).getConfig());	
		...
		if (iconf instanceof CCDConfig && name.equals("RATCAM")) {
			...
			done.setErrorString("Filter mismatch: Lower: "+lfilt+(fail1 ? " fail": " ok")+
					    "Upper: "+ufilt+(fail2 ? " fail": " ok"));
		...
		ngat.message.ISS_INST.OFFSET_FOCUS df = new ngat.message.ISS_INST.OFFSET_FOCUS(request.getId());
		...
		if (! dfdone.getSuccessful()) {
		    ...
		    done.setErrorString("The offset-focus failed due to: "+dfdone.getErrorString());
		    done.setSuccessful(false);
		    if (implementor != null)
			implementor.sendDone(done);	
		    return;
		...
		// Send a MoveFold stow
                ...
                ngat.message.ISS_INST.MOVE_FOLD mf = new ngat.message.ISS_INST.MOVE_FOLD(request.getId());
                ...
                if (! mfdone.getSuccessful()) {
                    ...
                    done.setErrorString("The move fold failed due to: "+mfdone.getErrorString());
                    done.setSuccessful(false);
                    if (implementor != null)
                        implementor.sendDone(done);
                    return;
		...
		try {Thread.sleep(5000L+expose);} catch (Exception e) {}
		...
		reply.setSuccessful(true);
		  ...
		  (request instanceof FRODOSPEC_LAMPFLAT) {
		ilf++;
		...
		reply.setSuccessful(true);
		  ...
		  (request instanceof FRODOSPEC_ARC) {
		ilf++;
		...
		reply.setSuccessful(true);
		  ...
		  (request instanceof FRODOSPEC_DARK) {
		ilf++;
		...
		if (now < runat) {
		    System.err.println("Wait for runat time in: "+((runat-now)/1000)+"s");
	
		    ack = new ACK("");
		    ack.setTimeToComplete((int)(runat - now) + 60000);
		    implementor.sendAck(ack);	

		    try {Thread.sleep(runat - now);} catch (Exception e) {}
		} else {
		    System.err.println("Passed runat time, starting exposure  immediately");
		}
		...
		try {Thread.sleep(duration+10000L);} catch (Exception e) {}
		...
		(request instanceof TELFOCUS) {

		double startFocus = (double)(((TELFOCUS)request).getStartFocus());
		double endFocus   = (double)(((TELFOCUS)request).getEndFocus());
		double step       = (double)(((TELFOCUS)request).getStep());
		double exposureTime = (double)(((TELFOCUS)request).getExposureTime());
		
		// work out what we will use for the optimum focus value 
		// somewhere near the centre of the range.
		double focus0 = 0.5*(endFocus + startFocus) + 0.2*(Math.random()-0.5)*(endFocus - startFocus); 
	
		// more model parameters
		double mk  = 3.0 + Math.random()*3.0;   // scaling factor. (3, 6)
		double mw0 = 0.2 + Math.random()*0.6;   // fwhm at optimal. (0.2, 0.8)
		...
		while (focus < endFocus) {
		    ...
		    ngat.message.ISS_INST.SET_FOCUS setfocus = new ngat.message.ISS_INST.SET_FOCUS(request.getId());
			...
			done.setErrorString("The telfocus failed due to: "+sfdone.getErrorString());
			done.setSuccessful(false);
			if (implementor != null)
			    implementor.sendDone(done);	
			return;
		    ...
		    // Telfocus Ack
		    try {Thread.sleep(3000+(long)exposureTime);} catch (Exception e) {}
		...
		// number = 0  MEANS a DURATION exposure, overall time is exposure time.
		...
		for (int ix = 0; ix < number; ix++) {
			...
			if (fits.getValue() == null) {
			    System.err.println("NULL Header: "+fits.getKeyword());		
			    done.setSuccessful(false);
			    done.setErrorNum(100999);
			    done.setErrorString("Null header for: "+fits.getKeyword());
			    if (implementor != null) 				    
				implementor.sendDone(done);	
			    return;
			}
			...
			mack.setFilename("/icc/tmp/"+filePrefix+affix+datestr+"_"+(im)+"_"+ix+"_1_0.fits");			   
			implementor.sendAck(mack);	
		    }
		    
		    // dump the fits
		    try {
		    File file = new File("/icc/tmp/"+filePrefix+affix+datestr+"_"+im+"_"+ixx+"_1_0.fits");
		...
		for (int ix = 0; ix < number; ix++) {
			...
			mdack.setFilename("/icc/tmp/"+filePrefix+affix+datestr+"_"+(im)+"_"+ix+"_1_1.fits");	
			double seeing = avSee + Math.random()*deltaSee - 0.5*deltaSee;
		...
		MULTRUN_DONE done = new MULTRUN_DONE("");
		done.setFilename("/icc/tmp/"+filePrefix+affix+datestr+"_"+im+"_"+ixx+"_1_0.fits");
		double seeing = avSee + Math.random()*deltaSee - 0.5*deltaSee;
		...
		reply.setSuccessful(true);
		  ...
		  (request instanceof FRODOSPEC_MULTRUN) {
		...
		if (System.getProperty("dilate") != null) {
		    ...
		    System.err.println("Exposure: Using dilation factor: "+factor+" Expose-> "+expose);
		}
		...
		(request instanceof TWILIGHT_CALIBRATE) {
		...
		(request instanceof GET_STATUS) {
		GET_STATUS_DONE done = new GET_STATUS_DONE("");
		done.setCurrentMode(GET_STATUS_DONE.MODE_IDLE);
	
		// read any new status from a file which operator can change at will	
		Properties props = new Properties();	
		try {
		    props.load(new FileInputStream(name.toLowerCase()+".status")); // e.g. ratcam.status
		} catch (Exception e) {
		    // dont really care 
		}
		...
		status.put("Heater ADU", new Double(Math.random()*50.0+2500.0));

		if (name.equals("FRODO")) {
		    status.put("red.Temperature", new Double(20.0 + Math.random()*5.0));
		    status.put("red.Heater ADU", new Double(Math.random()*50.0+2500.0));
		    status.put("blue.Temperature", new Double(40.0 + Math.random()*5.0));
		    status.put("blue.Heater ADU", new Double(Math.random()*50.0+2500.0));
		    status.put("Environment.Temperature.0",  ""+(Math.random()*50.0)); // bizzarely a string
		    status.put("Environment.Temperature.2", ""+(Math.random()*50.0));  // bizzarely a string
		} else if
		      (name.equals("RINGO3")) {
		    status.put("Temperature.0.0", new Double(10.0 + Math.random()*2.0));
		    status.put("Temperature.1.0", new Double(15.0 + Math.random()*2.0));
		    status.put("Temperature.1.1", new Double(20.0 + Math.random()*2.0));
		}
		...
		reply.setErrorString("Unknown command class: "+request.getClass());		
	    }
	...
	public void failedConnect(Exception e) {
	    e.printStackTrace();
	    done = failed(7501, "Failed to connect to ISS@"+issHost+":"+issPort+" for command: "+command);
	}
	...
	public void handleAck(ACK ack) {
	    System.err.println("ICS: Ack:"+ack+" to="+ack.getTimeToComplete());
	}
